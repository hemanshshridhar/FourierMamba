...
class SS2D_Channel(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        x_hwwh = x.view(B, -1, C)
        xs = torch.cat([x_hwwh, torch.flip(x_hwwh, dims=[-1])], dim=1).view(B, 2, -1, C) # (1, 4, 192, 3136)
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs.view(B, K, -1, L), self.x_proj_weight)
        dts, Bs, Cs = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=2)
        dts = torch.einsum("b k r l, k d r -> b k d l", dts.view(B, K, -1, L), self.dt_projs_weight)
        ...
        inv_y = torch.flip(out_y[:, 1], dims=[-1]).view(B, 1, -1, C)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        assert y1.dtype == torch.float32
        y = y1 + y2 
        y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B,-1,C)
        y = self.out_norm(y)
        y = y * F.silu(z)
        out = self.out_proj(y)
        if self.dropout is not None:
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.attention2 = SS2D_Channel(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, **kwargs)
